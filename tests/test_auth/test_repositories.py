...
class TestUserRepository:
    ...
    async def test_create_user_if_already_exists(self, set_session, create_user):
        await create_user('test_user', 'test_password')
        with pytest.raises(UsernameAlreadyExists):
            await user_repository.create_user('test_user', 'hashed_password')

    async def test_create_user_success(self, set_session, create_user):
    ...
    async def test_get_user_by_username(self, set_session, create_user, user_exists: bool):
        if user_exists:
            await create_user('test_user', 'test_password')
